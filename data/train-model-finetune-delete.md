...
Delete Dreambooth Training \| Stable Diffusion API Documentation
...
* [Train Model](/docs/category/train-model)
* Delete Training
On this pageDelete Dreambooth Training
==========================
...
```
Send a `POST` request to [https://stablediffusionapi.com/api/v3/finetune/delete/{training\_id}](https://stablediffusionapi.com/api/v3/finetune/delete/%7Btraining_id%7D) endpoint. Where `{training_id}` is the one returned in the response upon initiating the model training using the [Create Dreambooth Request](/docs/train-model/finetune#response) endpoint.
...
curl_setopt_array($curl, array(  
  CURLOPT_URL => 'https://stablediffusionapi.com/api/v3/finetune/delete/F5jvdzGnYi',  
...
var options = {  
  'method': 'POST',  
  'url': 'https://stablediffusionapi.com/api/v3/finetune/delete/F5jvdzGnYi',  
  'headers': {  
    'Content-Type': 'application/json'  
  },  
  body: JSON.stringify({  
...
```
[PreviousGet Model List](/docs/train-model/finetune-list)[NextCancel Training](/docs/train-model/cancel-training)* [Overview](#overview)
* [Request](#request)
